...
class ReaderThread (Thread):
	...
	def run(self):
		self.keyString = "# Begin JT's Project"

		self.port = serial.Serial('/dev/tty.usbmodem1411')	   # open the serial port (MAC/Linux)

		while 1:
			channelList = []
			while 1:
				s = self.port.readline().strip()			 # read a line of input from the Arduino
				if s=='***':
					if len(channelList)==self.numchannels:
						self.recvQueue.put(channelList)
						channelList=[]
					else:
						print "Ack! I got the stars but not enough data", len(channelList)
						channelList=[]
				else:
					try:
						channelList.append(int(s.strip()))
					except ValueError:
						print "Ack. conversion error:" + `s`, channelList
...
class MonitorThread(Thread):
	...
	def __init__(self, callback=None, fr=None, nc=1024):
		...
		self.fr = fr
		self.numchannels = nc
		log("Monitor thread inited with callback=" + `callback`)
		
	def run(self):
		
		self.reader = ReaderThread(nc=self.numchannels)
